...
#ighv <- read.csv(file="/home/almut/Documents/CLL/results/deseq/IGHV_Tsig_Tri12/IGHV_diffGenes.csv")
ighv <- read.csv(file="/home/almut/Documents/CLL/results/deseq/multifactor/IGHV_diffGenes.csv")
...
int_genes <- ighv[which(ighv$padj < 0.01 & abs(ighv$log2FoldChange) > 4),]
...
annocolor <- list(IGHV = c("M" = annocol[11], "U" = annocol[6]), Methylation = c("IP" = annocol[3], "LP" = annocol[4], "HP" = annocol[2]),trisomy12 = c("1" = annocol[7], "0" =annocol[8]))
...
ha_col <- HeatmapAnnotation(colData(ddsCLL)[,c("IGHV", "Methylation", "trisomy12")], col = annocolor, annotation_height = unit(c(rep(1.3, 3)), "cm"), annotation_legend_param = list(title_gp = gpar(fontsize = 20), labels_gp = gpar(fontsize = 15),  grid_height = unit(0.7, "cm"), grid_width = unit(0.7, "cm"), gap = unit(5, "mm")))
...
ha_genes <- rowAnnotation(link = row_anno_link(at = geneIDs, labels = labels, labels_gp = gpar(fontsize = 25)), width = unit(8, "cm"))

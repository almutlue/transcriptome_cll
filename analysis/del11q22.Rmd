...
#differentially expressed genes between del11q22.3 groups (see differential expression.html)
...
mutStatus <- data.frame(colData(ddsCLL)) %>% arrange(del11q22.3)
...
```{r complex heatmap, fig.width= 22, fig.height=30}

#colors
colors = colorRamp2(c(-4,-1,0,1,4), c("#2166ac","#4393c3", "#f7f7f7", "#d6604d","#b2182b"))
annocol <- get_palette("jco", 10)
annocolor <- list(del11q22.3 = c("1" = annocol[8], "0" = annocol[9]))
...
saveRDS(h1, file = paste0(output_dir, "/figures/r_objects/del11q22/del11q22_heatmap.rds"))
```
...
gene_count <- function(gene_nam){
  ...
  saveRDS(p, file = paste0(output_dir, "/figures/r_objects/del11q22/de_genes/", gene_nam, ".rds"))
  p
}
...
#Filter chromosome 11 - we are not interested in the dosage effect 
diff_res <- diff_res[-which(diff_res$chromosome %in% c("11")),]
...
enrichPlot <- resPlot[c(1:6),] %>% mutate(log10Padj = -log10(p.adj)) 
...
saveRDS(p, file = paste0(output_dir, "/figures/r_objects/del11q22/del11q22_enrichment.rds"))

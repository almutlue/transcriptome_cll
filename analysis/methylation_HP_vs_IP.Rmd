...
#arrange columns
mutStatus <- data.frame(colData(ddsCLL)) %>% arrange(Methylation) %>% filter(!is.na(Methylation)) %>% filter(Methylation %in% c("IP", "HP"))
...
ggsave(file=paste0(figure_dir, "/pca_Meth_HPIP_top150.svg"), plot=p, width=5, height=5)
```
...
saveRDS(diff_meth, file= paste0(output_dir,"/diff_meth_HP_IP.rds"))
...
write.csv(dataTab, file=paste0(output_dir, "/diff_genes/meth_HP_IP_diffGenes.csv"))
...
#svg(filename=paste0(figure_dir, "/gene_expr_Methylation_HPIP.svg"), width=30, height=35)
#pdf(file=paste0(figure_dir,"/gene_expr_Methylation_HPIP.pdf"), width=30, height=45)
draw( h1 + ha_genes)
#dev.off()
...
gene_count <- function(gene_nam){
 ...
 # ggsave(file=paste0(figure_dir, "/methylation_HPIP/genetic_interaction_", gene_nam, ".svg"), plot=p, width=6, height=5)
  p
}
...
ggsave(file=paste0(figure_dir, "/GSEA_Meth_HPIP_Kegg.svg"), plot=p, width=14, height=7)

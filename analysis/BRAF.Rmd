...
diff_fil <- diff_all[which(diff_all$padj < 0.01 ),-1]
diff <- diff_fil[which(abs(diff_fil$stat) > 4.5),]
...
rownames(feature) <- colnames(ddsCLL)
...
diff <- diff_fil[which(abs(diff_fil$stat) > 5 & abs(diff_fil$log2FoldChange) > 2),]
...
draw(h1) 
#dev.off()
saveRDS(h1, file = paste0(output_dir, "/figures/r_objects/BRAF/BRAF_heatmap.rds"))
```
...
gene_count <- function(gene_nam){
  ...
  saveRDS(p, file = paste0(output_dir, "/figures/r_objects/BRAF/de_genes/", gene_nam, ".rds"))
  p
}
...
Run piano
```{r piano, fig.width=6, fig.height=6}
...
p <- ggbarplot(enrichPlot, x = "pathway", y = "gene_number",
          ...
          label = enrichPlot$pathway, lab.col = "white", lab.size = 5, lab.vjust = 0, lab.hjust = 1.01,
          sort.val = "none",         
...
saveRDS(p, file = paste0(output_dir, "/figures/r_objects/BRAF/BRAF_enrichment_dn.rds"))
...
saveRDS(p, file = paste0(output_dir, "/figures/r_objects/BRAF/BRAF_enrichment_up.rds"))
```

...
diff <- diff_all[which(abs(diff_all$log2FoldChange) > 2 & diff_all$baseMean > 200),]
...
annocolor <- list(IGHV = c("M" = annocol[8], "U" = annocol[9]))
...
draw(h1 ) 
#dev.off()
saveRDS(h1, file = paste0(output_dir, "/figures/r_objects/ighv/ighv_heatmap.rds"))
```
...
diff <- diff_all[which(abs(diff_all$log2FoldChange) > 3 & diff_all$baseMean > 200),]

...
gene_count <- function(gene_nam){
  ...
  p <- ggstripchart(gc, x = variant, y = "count",
          color = variant,
          size = 2.5,
          palette = "jco",
          add = "boxplot",
...
dotplot(em2, showCategory=10) + ggtitle("GSEA for IGHV")
dotplot(em, showCategory=10) + ggtitle("Enrichment for IGHV")

dotplot(kk2, showCategory=10) + ggtitle("GSEA for IGHV")
dotplot(kk, showCategory=10) + ggtitle("Enrichment for IGHV")
...
#Networks
cnetplot(em2, categorySize="pvalue", foldChange=gene_lfc)
cnetplot(kk2x, categorySize="pvalue", foldChange=gene_lfc)
```
...
Run piano
```{r piano, fig.height=6, fig.width=8}
...
saveRDS(p, file = paste0(output_dir, "/figures/r_objects/ighv/ighv_enrichment_dn.rds"))
...
saveRDS(p, file = paste0(output_dir, "/figures/r_objects/ighv/ighv_enrichment_up.rds"))
```

# Expression of specific pathways 

```{r pathway heatmap, fig.height= 18, fig.width= 14.22}

plot_pw <- function(pw_genes, pathway_name){
  #filter for pw genes in variant
  exprMat <- assay(RNAnorm)
  rownames(exprMat) <- rowData(RNAnorm)$symbol
  pw_complete <- pw_genes[pw_genes %in% rownames(exprMat)]
  message(paste(length(pw_complete), " genes from ", length(pw_genes), " found."))
  exprVariant <- exprMat[pw_complete,]
  colnames(exprVariant) <- colData(ddsCLL)$PatID
  exprVariant.new <- log2(exprVariant)
  exprVariant.new <- t(scale(t(exprVariant.new)))
  exprVariant.new[exprVariant.new > 4] <- 4
  ...
  ha_col <- HeatmapAnnotation(df = feature, col = annocolor, 
                              annotation_height = unit(c(rep(1.9, 1)), "cm"), 
  ...
  p1 <- draw(h1) 
  saveRDS(p1, file = paste0(output_dir, "/figures/r_objects/ighv/ighv_", pathway_name, ".rds"))
  p1
}

p1 <- plot_pw(pw_genes = gsc$gsc[["HALLMARK_TNFA_SIGNALING_VIA_NFKB"]], pathway_name = "HALLMARK_TNFA_SIGNALING_VIA_NFKB")
p2 <- plot_pw(pw_genes = gsc_Kegg$gsc[["KEGG_B_CELL_RECEPTOR_SIGNALING_PATHWAY"]], pathway_name = "KEGG_B_CELL_RECEPTOR_SIGNALING_PATHWAY")
p3 <- plot_pw(pw_genes = gsc$gsc[["HALLMARK_MTORC1_SIGNALING"]], pathway_name = "HALLMARK_MTORC1_SIGNALING")

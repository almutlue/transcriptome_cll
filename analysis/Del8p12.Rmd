...
#differentially expressed genes between del8p12 groups (see differential expression.html)
...
mutStatus <- data.frame(colData(ddsCLL)) %>% arrange(del8p12)
...
annocolor <- list(del8p12 = c("1" = annocol[8], "0" = annocol[9]))
...
saveRDS(h1, file = paste0(output_dir, "/figures/r_objects/del8p12/del8p12_heatmap.rds"))
```
...
gene_count <- function(gene_nam){
  ...
  saveRDS(p, file = paste0(output_dir, "/figures/r_objects/del8p12/de_genes/", gene_nam, ".rds"))
  p
}
...
Run piano
```{r piano, eval=FALSE, fig.height=6, fig.width=8, include=FALSE}
...
saveRDS(p, file = paste0(output_dir, "/figures/r_objects/del8p12/del8p12_enrichment_dn.rds"))
...
saveRDS(p, file = paste0(output_dir, "/figures/r_objects/del8p12/del8p12_enrichment_up.rds"))
```
